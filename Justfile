...
crap-ner:
    ...
    COVERAGE_FILE=/tmp/osm-polygon-wikidata-ner-crap-coverage-$$ UV_CACHE_DIR={{UV_CACHE_DIR}} uv run pytest -q tests/ner/test_job.py tests/ner/test_otter.py tests/ner/test_pipeline.py tests/ner/test_publication.py tests/ner/test_silver_evaluator.py tests/ner/test_silver_validation.py tests/ner/test_wikineural.py tests/grid5000/test_ner_controller.py tests/ner/test_pilot.py tests/ner/test_crosscheck_pilot.py --cov=osm_polygon_wikidata_only.ner --cov=osm_polygon_wikidata_only.grid5000.ner_controller --cov=scripts.grid5000_geographic_ner --cov=scripts.prepare_geographic_ner_pilot --cov=scripts.prepare_geographic_ner_crosscheck --cov=scripts.evaluate_geographic_ner_pilot --cov-branch --cov-fail-under=0 --cov-report=json:/tmp/osm-polygon-wikidata-ner-crap-coverage.json
    UV_CACHE_DIR={{UV_CACHE_DIR}} uv run radon cc -j src/osm_polygon_wikidata_only/ner/job.py src/osm_polygon_wikidata_only/ner/otter.py src/osm_polygon_wikidata_only/ner/pipeline.py src/osm_polygon_wikidata_only/ner/publication.py src/osm_polygon_wikidata_only/ner/silver_validation.py src/osm_polygon_wikidata_only/ner/wikineural.py src/osm_polygon_wikidata_only/grid5000/ner_controller.py scripts/grid5000_geographic_ner.py scripts/prepare_geographic_ner_pilot.py scripts/prepare_geographic_ner_crosscheck.py scripts/evaluate_geographic_ner_pilot.py > /tmp/osm-polygon-wikidata-ner-crap-complexity.json
