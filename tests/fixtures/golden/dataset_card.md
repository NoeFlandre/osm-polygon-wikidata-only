...
This is the V1 dataset, generated by the Wikidata-first V1 contract in the [V1 code on GitHub](https://github.com/NoeFlandre/osm-polygon-wikidata-only/tree/v1.0.0). V1 selects polygons with a valid `wikidata=*` reference. The separate V2 dataset adds direct multilingual `wikipedia=*` discovery without changing this V1 dataset.
