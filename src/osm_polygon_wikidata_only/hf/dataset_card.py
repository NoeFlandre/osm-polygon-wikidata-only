...
def render_dataset_card(
    *,
    ...
    body = (
        ...
        "Colour encodes the share of dataset polygons (already conditional on "
        "an OSM `wikidata=*` tag) linked to at least one Wikipedia article with "
        "non-empty text, from 0% to 100%. The denominator is every dataset "
        "polygon; the numerator counts unique polygons (never polygon-article "
        "links). Grey cells hold fewer than 20 polygons and are not "
        "statistically meaningful. Cell opacity does not encode polygon count.\n\n"
        "### Polygon density\n\n"
        "![Geographic Polygon Density](assets/geographic_polygon_count.png)\n\n"
        "Colour encodes the number of dataset polygons per H3 cell on a "
        "logarithmic scale. Each dataset polygon (conditional on an OSM "
        "`wikidata=*` tag) is counted exactly once. Low counts are the metric "
        "and remain visible; opacity is not used as a second data encoding.\n\n"
        f"{stats_block}\n"
        f"{schema_section}\n"
