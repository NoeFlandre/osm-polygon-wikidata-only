"""Per-PBF row construction: enrich polygons and build articles + links.

This module owns the focused, deterministic construction of the
three dataset rows derived from one enriched PBF:

* :func:`enrich_polygon` -- derive a polygon's coverage fields from
  its already-fetched ``LinkSummary``.
* :func:`build_articles_and_links` -- deduplicate articles by stable
  identifier and build the polygon->article link rows in source
  order.
* :func:`article_row` -- assemble one :class:`Article` from a
  ``WikipediaArticle`` plus its ``LinkSummary`` and entity labels.

The orchestrator (:mod:`pipeline.processor`) calls these in sequence
between the enrichment phase (:mod:`pipeline.enrichment_phase`) and
the persistence phase (:mod:`pipeline.persistence`).

For backwards compatibility, :mod:`pipeline.rows` re-exports the
three helpers by identity so older imports keep working. Text
cleaning helpers are sourced from :mod:`pipeline.rows` so existing
tests can still monkeypatch them through the historical location.
"""
